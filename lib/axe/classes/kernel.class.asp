...
class Kernel
    ...
    public sub process()
        ...
        if(fileExists(Server.mapPath(vpController))) then
            ...
            addError(strsubstitute("Controller '{0}' is not available. Looking for '{1}'", array(Session("controller"), vpController)))
        end if
    end sub
    ...
    public sub createFile(sFilePath, sContent)
        ...
        Stream.writeText(sContent & vbNewLine)
        Stream.writeText("<!--// CACHED FILE => Execution time is less than 1µs (Blazing fast performance) //-->")
        Stream.setEoS()
        Stream.position = 0
