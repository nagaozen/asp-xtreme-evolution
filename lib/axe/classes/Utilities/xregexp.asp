...
AXE(ASP Xtreme Evolution) XRegExp is a raw port of Steven Levithan XRegExp.

XRegExp supports all native ES5 regular expression syntax.

License:

This file is part of ASP Xtreme Evolution.
Copyright (C) 2014 Fabio Zendhi Nagao
...
augmented and extensible regular expressions. You get new syntax, flags, and 
methods beyond what ES3 and VBScript 5.5 support natively. XRegExp is also a 
regex utility belt with tools to make your grepping simpler and more powerful.

Notes:

    - A raw version of <http://xregexp.com/>.
    - Take a look at <http://xregexp.com/api/> for the API.

About:

    - Written by Fabio Zendhi Nagao <http://zend.lojcomm.com.br/> @ Jun 2014
...
Creates an extended regular expression object for matching text with a pattern.
Differs from a native regular expression in that additional syntax and flags are
supported. The returned object is in fact a native RegExp and works with all
native methods.

Parameters:

    (string) - Regex pattern string, or an existing regex object to copy.
    (string) - Any combination of flags.
...
Executes a regex search in a specified string. Returns a match array or null. If
the provided regex uses named capture, named backreference properties are 
included on the match array. Optional pos and sticky arguments specify the 
search start position, and whether the match must start at the specified 
position only. The lastIndex property of the provided regex is not used, but is 
...
Parameters:
    ...
    (number) - Zero-based index at which to start the search.
    (mixed)  - Whether the match must start at the specified position only. The string 'sticky' is accepted as an alternative to true.

Returns:

    (mixed) - Match array with named backreference properties, or null.
